/* ============================================================
   DIGIYLYFE · MARKET · ABOS ACCESS BRIDGE
   À poser dans : assets/js/digiy-abos-access.js

   Rôle :
   - Préparer MARKET au rail commun ABOS / PAY / ADMIN.
   - Lire l’accès module via :
     public.digiy_has_module_access_from_abos(phone, module)

   Important :
   - MARKET est encore "à cadrer" dans le catalogue.
   - Ne pas bloquer MARKET avec protect() tant que le plan n’est pas actif.
   ============================================================ */
...
(function () {
  ...
  function savePhone(phone, module) {
    ...
    try {
      ...
      if (m === "MARKET") {
        localStorage.setItem("DIGIY_MARKET_PHONE", p);
        localStorage.setItem("DIGIY_MARKET_LAST_PHONE", p);
      }
    } catch (_) {}
  }
  ...
  function renderAccessBadge(target, result) {
    ...
    if (result.has_access) {
      el.innerHTML = `
        ...
        ${result.fiche_title || result.module || "MARKET DIGIY"}<br>
        <small>Expire : ${result.expires_at || "date suivie par DIGIY"}</small>
      `;
    } else {
      el.innerHTML = `
        <strong>🔒 MARKET à cadrer</strong><br>
        <small>Le pont ABOS est prêt. PAY garde la preuve, ADMIN validera quand le plan MARKET sera actif.</small>
      `;
    }
  }
